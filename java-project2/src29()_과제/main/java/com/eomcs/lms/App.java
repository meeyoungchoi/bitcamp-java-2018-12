...
public class App {
  ...
  private static void loadLessonData() {
    try (DataInputStream in = new DataInputStream(
    		...
    		new FileInputStream("lesson1.data")))) {//여기에 선언된 변수는 자동으로 close를 해주기 때문에 굳이 close를 또 써줄 필요가 없다.
  ...
  private static void saveLessonData() {
    try ( 
        DataOutputStream out = new DataOutputStream(
        		new BufferedOutputStream(new FileOutputStream("lesson1.data")));) {
      
    	out.writeInt(lessonList.size());//몇번 입력이 되었는지가 사이즈에 저장된다.
  ...
  private static void loadMemberData() {
    ...
    try(DataInputStream in = new DataInputStream(
   		 new BufferedInputStream(
				 new FileInputStream("member1.data")));) {
    ...
    try (
        DataOutputStream out = new DataOutputStream(
        		new BufferedOutputStream(
        				new FileOutputStream("member1.data")));) {//AUTOCLOSABLE
  ...
  private static void loadBoardData() {
   try(DataInputStream in = new DataInputStream(
		   new BufferedInputStream(
				   new FileInputStream("board1.data")))) { 
  ...
  private static void saveBoardData() {
    try (
        DataOutputStream out = new DataOutputStream(
        		new BufferedOutputStream(
        				new FileOutputStream("board1.data")))) {
