...
//BoardDaoImpl 클래스로 변경
//3.BOardDodImpl 객체를 서버쪽에서 대행할 클래스를 만든다.
//boardServeice의 이름을 BOardDaoskel로 변경한다.
//4/ 클라이언트 쪽에서 BoardDaoImpl 객체를 대행할 프록시 클래스를 만든다.
//BoardAgent 클래스를 BoardDaoProxy 클래스로 변경한다.
//BoardDaoProxy 클래스는 BoardDaoImpl 클래스와 마찬가지로 BoardDao 인터페이스를 구현한다.
//패키지명도 agent 대신 프록시로 변경한다.
