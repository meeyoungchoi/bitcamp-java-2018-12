...
public class LessonHandler {
	  ...
	  LessonList list;//LessonList 타입의 list 인스턴스 생성
	  Scanner keyboard;
	  
	  public LessonHandler(Scanner keyboard) {//기본생성자 대신에 파라미터값으로 keyboard를 받는 생성자를 추가한다.
		  this.keyboard = keyboard;
		  this.list = new LessonList(20);//LessonList() 생성자를 호출하려면 인스턴스를 생성해야 하므로 인스턴스를 생성하였다.
	  }
	 ...
	 public void listLesson() {//수업 데이터 배열을 리턴
		 Lesson[] lessons = list.toArray();//인스턴스 메서드 toArray()에 접근
