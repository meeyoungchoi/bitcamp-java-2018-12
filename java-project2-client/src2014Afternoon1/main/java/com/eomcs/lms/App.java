...
public class App {
	...
	public void service() {
		...
		while (true) {
			...
			if (commandHistory.equals("quit")) {
				System.out.println("종료합니다.");
			} else if (commandHistory.equals("commandHistory2")) {
				
				
			} else if (commandHistory.equals("commandHistory")) {
			...
			//stateful을 stateless로 전환할때 주의할 점
			//가능한 서버에 요청하는 시점에 서버와 연결하라
			//이 클래스에서 서버와 연결하지 않고 데이터를 요청하는 역할을 수행하는 
			//객체 (Agent)에 서버 연결을 맡긴다.
