  ...
  private Object getObjectValue(Class<?> paramType, HttpServletRequest request) {
   ...
   /* Member member = new Member();
    ...
    try {
      ...
      //=>만약 paramType이 Mebmer 클래스라면,
      //Object obj = new Member();
      Object obj = paramType.getConstructor().newInstance(); //어떤 객체를 생성해야하는가

      //파라미터 타입의 세터 메서드를 알아낸다.
      //객체의 프로퍼티 이름과 같은 요청 파라미터 값을 찾아 젖아한다.
      //=>obj.setTitle(reqeust.getParameter("title"));
      //=>obj.setPassowrd(request.getParameter("password"));
      //
      ...
      for (Method method : methods) {
        ...
        //예) setRegisteredDate() => egisteredDate
        String propName = method.getName().substring(3,4).toLowerCase() +//3이상 4미만 즉 "r" + "egisteredDate" => registeredDate 
            method.getName().substring(3);

        //프로퍼티 일므으로 요청 파라미터 값을 찾아서 세터를 호출한다.
        //=>메서드의 파라미터 타입을 알아낸다.
        Class<?> type = method.getParameterTypes()[0]; //세터의 파라미터는 오직 

        //=> 요청 파라미터에서 프로퍼티 타입과 일므이 같은 값을 꺼낸다.
        try {
