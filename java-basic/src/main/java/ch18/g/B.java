...
public interface B {
  ...
  //=> 기존 규칙에 새 메서드를 넣으면 그 규칙에 따라 작성했던 My1과 My2 클래스에도 새 메서드를 추가해야 한다.
  //그런데 기존 시스템을 사용하는 고객은 그 기능이 필요 없다.
  //시스템을 업그레이드 한다면 모를까
  //=>이런 상황일때 예전에는 B인터페이스를 상속 받아 새 메서드를 추가한 서브 인터페이스를 만들었다.
  // 문제는 새 인터페이스를 만들게 되면 기존 코드도 변경해야 하는 번거로움이 있었다.
  //=>이런 문제를 해결하기 위해 등장한 문버이 default 메서드이다.
  //즉 인터페이스도 구현 메서드를 가질 수 있다.
  
  //다음과 같이 새 추상메서드를 추가하면 기존의 My1, My2 클래스에서 오류가 발생한다.
  //void m2();//새 추상메서드를 추가한 순간 My1, My2.java가 오류난다.
  
  //디폴트 메서드를 만들라!
  //=>구현된 메서드이기 때문에 B를 구현한 이전 My1과 My2는 영향을 받지 않는다.
  //이것이 디폴트 메서드의 목적이다.
  //물론 구현 메서드에 간단한 코드를 작성할 수 있지만,
  //클래스 처럼 무너가 진짜 일을 하는 코드를 ㅈ가성해서는 안된다.
  default void m2() {};//인터페이스에 새 메서드를 추가하는것이 목적이다.
}
