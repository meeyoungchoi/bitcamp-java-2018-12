...
public class Canvas {
  ...
  //좀 더 추상적인 방식으로 클래스 이름 대신 인터페이스를 사용하여, 특정 규칙에 따라 만든 도구를 사용하겠다고 지정한다.
  ...
  //도구를 사요하는 쪽이나 도구를 만드는 쪽 모두 Monami로 제한되어 유연하지 못하다.
  //그러나 위의 필드 처럼 도구를 지정할 떄 인터페이스를 사용하면,
  //어느 클래스를 상속 받는지 상관없이 다양한 도구를 사용할 수 있어 
  //canvas를 만든 개발자 입장에서는 훨씬 편리하다.
  //또한 도구를 만드는 입장에서도 다른 유용한 클래스를 마음껏 상속 받아 도구를 만들수 있어
  //매우 편리하다. 어떤 클래스를 상속 받든 단지 Pen이라는 규칙만 따르면 되기 때문이다.
  //Monami tool;
  
  public Canvas(Pen pen) {
    this.tool = pen;
  }
