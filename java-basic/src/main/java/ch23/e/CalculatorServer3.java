...
public class CalculatorServer3 {
	public static void main(String[] args) {
		...
		try(ServerSocket serverSocket = new ServerSocket(8888)) {
			...
			//서버의 Stateful 응용`
			//작업 결과를 변수에 저장하여 연결을 끊을때 까지 계속 유지한다.
			//
			while (true) {
				...
				try (Socket socket = serverSocket.accept();
					...
					while (true) {
						...
						try {
							op = input[0];//연산자(0번방)
							b = Integer.parseInt(input[1]);//숫자 기존값에 계속 더한다 (1번방) 서버에서는 결과를 계속 유지해야 한다.

						} catch (Exception e) {
							out.println("식의 형식 바르지 않음");
							out.flush();
