...
public class Test02 {
	...
	Method m1 = s1.getClass().getMethod("setName", String.class);//메서드가 갖는 타입의 값을 나열하면 된다.
	
	//invoke(객체주소, 파라미터값,...)
	m1.invoke(s1, "임꺽정"); // 얘는 s1.setName("임꺽정");
	...
	//static 메서드는 인스턴스가 필요없다.
	//메서드의 리턴 값이 있다면 받으면 된다.
	//단 타입 캐스팅 문법을 붙여야 하나다.
