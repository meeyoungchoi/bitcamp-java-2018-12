...
public class Test03 {
  public static void main (String[] args) {
    ...
    //My3.ia = 500;//인스턴스 없이 접근할 수 없다.
  
    //클래스 필드에 접근할 때는 레퍼런스를 통해 접근할 수 있다.
    obj1.sa = 500;//ok
    // 인스턴스 필드에 obj가 있는지 찾아보고 없으면 obj1의 클래스에서 자동으로 찾는다.
    //컴파일 오류는 발생시키지는 않지만 다른 개발자가 코드를 읽을 때 인스턴스 필드로 오해할 수 있으므로
    //클래스 필드는 클래스 이름으로 사용해야한다.
