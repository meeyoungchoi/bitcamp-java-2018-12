...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    // Calculator2를 통해 Calculator의 기능을 호출할수 있다.
    //
 ...
 *
 * 
 * 2. 기존 코드를 그대로 복사하여 새 클래스를 만드는 방법
 * 장점:
 * -기존 코드를 손대지 않기 때문에 문제가 발생할 가능성은 낮다.
 * 단점:
 ...
 * -기존 코드에 버그가 있을 때 복사 붙여넣기 해서 만든 클래스도 영얗을 받는다.
 * -기존 코드를 변경했을때 복사 붙여넣기 한 모든 클래스를 찾아 변경해야한다.
 * 
 * 3. 기존 코드를 상속 받아 기능을 추가하는 방법
 * -장점
 *-기존 클래스의 소스 코드가 필요 없다.
 *-간단한 선언으로 상속 받겠다고 표시한 후 새 개능만 추가하면 된다.
 *-단점
 *-일부 기능만 상속 받을 수 없다.
