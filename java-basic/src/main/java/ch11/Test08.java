...
public class Test08 {
  public static void main(String[] args) {
   ...
   //어떤 클래스에 equals()가 있나요?
    ...
    //=> 따라서 자바의 모든 클래스는 Object 클래스에 정의도니 메서드를 사용할 수 있다.
    //
    ...
    //=> finalize(): 가비지 컬렉터에 의해 제거 되기 전에  호출된다.
    //=> wait(): 잠시 실행을 멈춘다. 외부에서 깨워줄 때까지 멈춘다.
    //=> notify() : wait()로 멈춘 코드를 다시 실행한다.
    //=> notifyAll(): wait()로 실행을 멈춘 것을 모두 깨운다.
    
    //오브젝트로 부터 상속 받은 equals()가 원래는 인스턴스가 같은지를 비교한다고 했는데,
    //왜, String 클래스는 인스턴스가 아닌 문자열 값을 비교하나요?
    //=> String 클래스는 상속 받은 equals()메서드를 재정의 하엿기 때문이다.
    //이렇게 상속 받은 기능을 자신의 역할에 맞게끔 재정의 하는 것을 오버라이딩 이라 한다.
    //
    //StringBuffer는 재정의 하지 않아서 다르다고 결과가 나온건가요?
    //=>예!(스트링버퍼는 재정의 하지 않아서 그렇다.)
    //mutable: 변경할수 있는  (스티링버퍼) / immutable: 변경할수 없는
