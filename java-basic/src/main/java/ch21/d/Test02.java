...
public class Test02 {
	...
	public static void main(String[] args) {
		addName("홍길동");
		addName("임꺽정");
		addName(null);//전학간 학생인 경우 해당 번호의 이름을 null로 설정한다.
		addName(null);//전학간 학생인 경우 해당 번호의 이름을 null로 설정한다.
		addName("유관순");
		
		//Exception 계열(RuntimeException 제외) 의 예외가 발생하는 메서드를 호출할 떄는
		//반드시 예외처리를 해야 한다.
		//try ~ catch로 처리하든, throws 절ㅇ르 메서드 선언부에 붙이든 둘 중 하나는 해야 한다.
		//
		for (int i = 0; i < list.size(); i++) {
				String name = getName(i);
				//예외 상황을 검사하기 위해 getName()의 리턴 값을 확인하낟.
		...
		int result = divide(19191919, -1);//오류? 정상? 정상적인 나누기임에도 불구하고//리턴값이 -19191919 이기 때문에 오류로 판단한다.
		if (result == -1919191919) {//리턴값이 -1919191919이기 때문에 오류로 판단한다.
