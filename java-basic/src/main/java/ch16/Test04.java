...
class Box2<T> {
  ...
  public Box2(T value) {//T라는 타입의 인스턴스 주소를 젖아할 필드
    this.value = value;
  } 
...
public class Test04 {
  public static void main(String[] args) {
    ...
    //new를 실행할 때는 궅이 또 타입을 지정할 필요가 없다. 생략해도 된다.
   ...
   // Box2<String> x4 = new Box<>(new Date()); 오류
    
    //제네닉을 사용하면 getValue() 메서드가 각 타입별로 존재하는 것처럼 사용할 수 있다.
    //즉 형변환 할 필요가 없다.
