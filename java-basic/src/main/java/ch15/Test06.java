...
class My6 {
  ...
  //Map에 값을 저장하는 key로 사용할 때 hashCode()를 오버라이딩 해라
  //보통 값이 같은지 비교할 때 equals()와 함께 사용된다.
  //그래서 hashCode()를 오버라이딩 할
...
public class Test06 {
  ...
  /*System.out.println("=======================================");
   ...
   //  빠르게 두 값이 같은지를 알아낼 수 있다.
   // -매우 낮은 확률이지만 데이터가 다르더라도 같은 정수 값이 나올 수 있다.
   // 당연하다. 큰 데이터를 특별한 계산 공식을 통해 4바이트 정수 값으로 표현한다는 것은
   //언제든 다른 데이터에 대해 같은 값이 나올 가능성을 내표하고 있다.
   //다만 확률이 얼마나 낮은가의 문제이다.
   //다른 데이터에 대해 같은 정수 값이 나올 확률이 낮을 수록 안신하고 사용할 수 있다.
   // -현실과 비교하면 주민번호와 같다.
   // 본인 여부를 확인할 때 주민 번호로 확인하면 매우 빠르게 결과를 알수 있다.
   //원래는 본인인지 알아내려면 DNA 검사를 해야 한다.
   //그러면 너무 시간이 오래 걸린다. 그런데 지문이나 주민번호를 사용하면 빠르게 본인 여부를 알 수 있다.
   // -이런 이류호 hash 값을 "디지털 지문"이라 부른다.
   // -목적
   //=> 데이터가 같으지 빠르게 비교하기 위함이다.
   // -응용
   //=> 본인 여부를 확인하는 인증서
   //=>파일의 위변조를 검사하는 용도. =>git에서 커밋할 때 고유번호를 붙이는데 바로 해시 값이다.
   //=>파일 공유사이트에서 파일을 구분할 때 해시 값 사용.
