//annotation
//=>클래스, 메서드, 필드, 로컬 변수에 붙이는 특별한 주석이다.
//파라미터도 결국 로컬변수이다.
//=>다른 주석과 달리 컴파일 이나 실행할 때 추출할 수 있다.
//=>애노테이션 문법이 사용되기 전에 일반 주석에 특별한 문법을 포함시켜 사용 했던 doclet이라는 기술이 있었다.
//=>일반 주석과 달리 속성 프로퍼티명 = 값 형태로 값을 다룰 수 있다.
//=>사용법
...
public class Test01 {
	...
	@MyAnnotation //필드에 붙일수 있다.
	static int a;
	
	@MyAnnotation int b;//애노테이션을 붙인 후에 바로 코드를 작성해도 된다.
	
	@MyAnnotation //메서드 선언에 붙일 수 있다.
	void m1(@MyAnnotation int p1, //파리미터(로컬변수)에 붙일 수 있다.
			@MyAnnotation String p2) {

		@MyAnnotation int local; //로컬 변수 선언에 붙일 수 있다.
		
		//@MyAnnotation System.out.println("okok"); => 일반 문장에는 붙일 수 없다.
