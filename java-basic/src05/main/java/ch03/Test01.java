...
public class Test01 {
  ...
  public static void main(String[] args) {
   ...
   //i3=2200000000;//이때 발생하는 오류는 리터럴 표현 오류이다.( i3=2200000000;)
    ...
    l1=9000000000000000000L;//900경/ 9경22경을 넘어가면 컴파일오류
   //l2=9900000000000000000L;//8바이트 리터럴 오류
    
    
   //4바이트 부동소수점을 담을 메모리 준비
    //float는 유효자릿수가 7자리
    //소주의 정수부분과 소수부분을 합친 자릿수가 7자리여야 한다. 
    float f1;
    float f2;
    float f3;
    //98765.43f에서 이 수가 부동소수점 임을 나타내기 위해 f를 붙인다.
    f1=98765.43f;
   ...
   // System.out.println(f2);
    ...
    //float는 유효자리수가 7자리 이므로 7자리를 넘어가는 수는 잘린다.
    //근데 여덟번째 자리에서 반올림이 발생하면 마지막 일곱번째 자리수에 반올림된 결과가 나오지만
    //여덟번째 자리가 반올림이 안되면 나머지 숫자는 잘리가 7자리 값이 그대로 출력된다
    
    //double은 유효자릿수가 15자리이다.
    double d3;
    d3=f1+f2; //두 부동소수점의 계산 결과가 4바이트 크기를 초과해서 결과가 이상하게 나온것 같아
    //더 큰 메모리를 준비 했다.
    //그래서 더블로 바꿔서 더하면 결과값의 손실이 없이 그대로 출력된다.
    //왜냐하먄 더블의 유효자릿수가 15자리 이기 때문이다.
