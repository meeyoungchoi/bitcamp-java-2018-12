// 24단계: Spring Java config정리,Spring의 BeanPostProcessor로 RequestMappingAnnotation처리
// => Spring java config 클래스를 역할별로 분리한다. 
// 
// 작업
// 1) AppConfig에서 데이터베이스 관련 설정 분리
// =>DatabaseConfig 클래스 생성
// 2)AppConfig에서 Mybatis관련설정 분리
//=>
//3)RequestMappingAnnotation 처리
//=>RequestMappingAnnotationBeanPostProcessor 생성
//4)RequestMappingHandlerMapping을 스프링 IoC 컨테이너 관리 대상에 포함시키낟.
//=>@Componenet 애노테이션을 붙인다.
