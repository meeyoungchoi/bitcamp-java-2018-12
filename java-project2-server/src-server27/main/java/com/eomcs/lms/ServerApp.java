...
//=>xml설정을 사용하면 pointcut 규칙으로 트랜잭션을 적용할 메서드를 간단히 짖어할 수 있다.
//그래서 실무에서 많이 사용한다.
//
// 작업
//1)트랜잭션을 설정하는 xml 파일을 준비한다.
//=>tx-context.xml
//2)기존에 서비스 클래스에 붙인 @Transaction 에노테이션을 모두 제거한다.
//=>
//3) xml에서 트랜잭션 advice를 설정할 경우 aop 라이브러리를 사용하기 때문에 프로젝트에 추가한다.
