...
// => @Transactionl 애노테이션을 이용하여 트랜잭션을 이용할 수 있다.
//
// 작업
//1)@Transactional애노테이션을 처리할 객체를 스프링 IoC 컨테이너 설정에 등록한다.
//=>java config클래스에 @EnableTransactionManagement 애노테이션을 붙인다.
//2) 트랜잭션을 적용할 서비스 클래스의 메서드에 @Transactional을 붙인다.
//=>왜? DAO 메서드에 안 붙이고 서비스 클래스의 메서드에 붙이는가?
// -dao 메서드들은 업무에 따라 단독으로 실행될 때가 있고,
//  다른 db 작업과 묶여서 실행될 때가 있기 때문이다.
// -예를 들어 photoboardDao의 delete() 메서드를 보라
//이 메서드는 단독으로 실행할 수도 있지만,
//보통 PhotoFileDao의 delete()과묶여서 실행될 때가 있다.
