// 19단계: 애노테이션 적용 
//=>애노테이션을 적용하여 Ioc 컨테이너가 객체를 만들때 클래스의 애노테이션에서 객체 이름 추출하기
//
//Ioc (Inversion of Control)
//=>제어의 역전, 제어의 역행, 역제어 등으로 표현한다,
//
//Ioc의 대표적인 예:
//1)DI(Dependency Injection) : 내부적으로 작성된 메서드에 따라서 자동적으로 호출된다.
//-의존 객체 주입
//-사용할 객체를 직접 만들지 않고 외부에서 주입 받아 사용하는 방식
//-외부에서 객체를 주입하기 때문에 주입하는 객체를 쉽게 고체할 수 있따.
//예를 들어 Command 구쳔체를 테스트 할 때 진짜 Dao를 주입하지 않고
//테스트용 Dao를 주입할 수 있다.
//그래서 단위 테스트 하기가 쉽다.
//2. EventLISTENER
//-보통 코드를 실행할 때 작성된 순서대로 위에서 아래로 실행한다.
//-메서드 호출 코드가 있다면 해당 메서드를 호출한다.
//그리고 호출이 끝나면 원래 위치로 이동하여 
//다음 코드를 실행한다.
//-어떤 메서드는 "키보드 클릭", "마우스 클릭" 등 특정 상태에 놓여지면
//자동으로 호출된다. 
// -이런 메서드를 보통 리스너(listener)라 부르고
//이렇게 순차적으로 실행되지 않고 
//특정 상황에 놓일 때 흐름에 역해하여 호출된다.
//-이런 메서드(리스너)도 ioc의 한 예이다.
//-직접 호출하는 것이 아니라 내부에 의해 호출되는 메서드라는 의미로 
//-콜 백 메서드(callback method)라 부르기도 한다.
//보통 줄여서 cb(특히 자바 스크립트에서 함수 레퍼런스를 선언할 때)라고 할 때가 있다.
//
//작업
