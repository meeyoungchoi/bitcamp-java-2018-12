// 12단계; 커넥션풀(FLYWEIGHT 디자인 패턴 응용)
//클라이언트 요청르 처리할 때마다 매번 DB커넥션을 생성한다면
//DB커넥션 생성에 비용(실행시간, 메모리)이 많이 든다.
//해결책?
//DB커넥션을 생성한 다음에 버리지 말고 보관했다가 다시 사용하는 것이다.
//
//작업
//1.ConnectionFactory 클래스릐 이름을 DataSource로 변경한다.
		...
		public void run() {
			...
			try (Socket socket = this.socket;
				...
				try {
					...
					//클라이언트 요청을 처리한 후 커넥션을 통해 작업한 것을 최종 처리한다.
					con.commit();
				} catch (Exception e) {
					try {
						con.rollback();
