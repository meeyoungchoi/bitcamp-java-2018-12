...
public class ServerApp {
  ...
  public void addApplicationContextListener(ApplicationContextListener listener) {
    listeners.add(listener);//인스턴스.메서드
    //add:arraylist에 들어있는 메서드
  }//리스너를 서버에 저장한다.
    ...
    try (ServerSocket ss = new ServerSocket(8888)) {
      ...
      while (true) {
        ...
        try (Socket socket = ss.accept();
            ...
            PrintWriter out = new PrintWriter(socket.getOutputStream())) {//printwriter: 출력
          ...
          // 클라이언트에게 응답하기
          Command commandHandler = (Command) context.get(request);//HashMap.get(): key를 너허어주면 값을 가져온다.
