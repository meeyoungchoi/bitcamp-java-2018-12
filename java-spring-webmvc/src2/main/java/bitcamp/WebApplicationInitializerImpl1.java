...
//3)/META-INF/services/javax.servlet.ServletContainerInitailizer
//파일을 찾는다.
//4) 그 파일에서 ServletContainerInitialiser 구현체를 알아낸다.
//5) 해당 구현체의 인스턴스를 생성한 후 onStrartup()을 호출한다.
//6) 만약 그 구현체가 보고 받고자 하는 타입이 있다면 해당 클래스를 찾아
//onStartup()을 호출할 때 파라미터로 넘겨준다.
//
//webApplicationINitailizer 호출 과정
//1) 서블릿 컨테이너(예: 톰캣 서버)를 시작한다.
//2)spring-web-x.x.x.RELEAESE
...
//5)SpringServletContainerInitializer는 DispatcherServlet servlet = new DispatcherServlet(ac);
...
public class WebApplicationInitializerImpl1 
  ...
  public void onStartup(ServletContext servletContext) throws ServletException {
    System.out.println("WebApplicationINitailizerImpl1.onStartUp()...호출됨!");
    
    
    //DispatcherServlet에서 사용할 스피링 IoC 컨테이너를 준비한다.
