// 요청 핸들러의 아규먼트 - 콘텐트를 직접 리턴하기
package bitcamp.app1;

import javax.servlet.http.HttpServletResponse;
import org.springframework.http.HttpEntity;
import org.springframework.http.HttpHeaders;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;

@Controller 
@RequestMapping("/c05_1")
public class Controller05_1 {

  
  //테스트:
  //http://l
  @GetMapping("h1") 
  @ResponseBody 
  public String handler1() {
    //리턴 값이 클라이언트에게 보내는 콘텐트 라면
    //메서드 선언부에 @ResponseBody를 붙인다.
    //=>붙인지 않으면 프론트 컨트롤러는 view URL로 인식한다.
    //=> . @ResponseBody 가 없으면 "<html><body>abc가각간</body></html>";를 jspurl로 인식한다
    //=>출력 콘텐트는 부라우저에서 기본으로 html로 간주한다.
    //단 한글은 iso-8859-1 문자표에 정의된ㄷ 코다가 아니기 때문에
    // => 출력 콘텐트는 기본이 text/plain;charset=ISO-8859-1이다.
    //그래서 html 태그를 인식하지 못한다.
    //또한 한글은 ISo8859-1 문자표에 정의된 코드가 아니기 때문에 '?' 문자로 변경된다. 
    return "<html><body><h1>abc가각간</h1></body></html>";
    
  }
  
  //테스트:
  //http://l
  //=> 리턴되는 콘텐트의 mime 타이보가 charset을 지정하고 싶다면
 //애노테이션의 produces 프로퍼티에 설정하라
  @GetMapping(value= "h2", produces="text/html;charset=UTF-8")
  @ResponseBody 
  public String handler2() {
    //리턴 값이 클라이언트에게 보내는 콘텐트 라면
    //메서드 선언부에 @ResponseBody를 붙인다.
    //=>붙인지 않으면 프론트 컨트롤러는 view URL로 인식한다.
    //=> . @ResponseBody 가 없으면 "<html><body>abc가각간</body></html>";를 jspurl로 인식한다
    // => 출력 콘텐트는 기본이 text/plain;charset=ISO-8859-1이다.
    //그래서 html 태그를 인식하지 못한다.
    //또한 한글은 ISo8859-1 문자표에 정의된 코드가 아니기 때문에 '?' 문자로 변경된다. 
    return "<html><body><h1>abc가각간</h1></body></html>";
    
  }
  

  //테스트:
  //http://l
  //=> 리턴되는 콘텐트의 mime 타이보가 charset을 지정하고 싶다면
 //애노테이션의 produces 프로퍼티에 설정하라
  @GetMapping("h3")
  @ResponseBody 
  public String handler3(HttpServletResponse response) {
    //httpservletresponse에 대해 다음과 같이 콘텥느 타입을 설정해보야 소용없다.
    response.setContentType("text/html;charset=UTF-8");
    return "<html><body><h1>abc가각간</h1></body></html>";
    
  }
  
  //테스트:
  //http://l
  //=> 리턴되는 콘텐트의 mime 타이보가 charset을 지정하고 싶다면
 //애노테이션의 produces 프로퍼티에 설정하라
  @GetMapping("h4")
  @ResponseBody 
  public HttpEntity<String> handler4(HttpServletResponse response) {
    //httpservletresponse에 대해 다음과 같이 콘텥느 타입을 설정해보야 소용없다.
    HttpEntity<String> entity = new HttpEntity(
        "<html><body><h1>abc가각간</h1></body></html>");
    
    //이 경우에도 출ㄹ겨할 때 iso-8859-1 문자표의 코드로 변환하여 출력한다.
    //그래서 한글은 ? 문자로 변환된다.
       
    return entity;
   
    
  }
  
  //테스트:
  //http://l
  //=> 리턴되는 콘텐트의 mime 타이보가 charset을 지정하고 싶다면
 //애노테이션의 produces 프로퍼티에 설정하라
  @GetMapping(value="h5", produces="text/html;charset=UTF-8")
  @ResponseBody 
  public HttpEntity<String> handler5(HttpServletResponse response) {
    //한글을 제데로 출력하고 싶으면 위 애노테이션의 produces 속성에 콘텐트 타입을 지정한다.
    HttpEntity<String> entity = new HttpEntity(
        "<html><body><h1>abc가각간</h1></body></html>");
    
    //이 경우에도 출ㄹ겨할 때 iso-8859-1 문자표의 코드로 변환하여 출력한다.
    //그래서 한글은 ? 문자로 변환된다.
       
    return entity;
   
    
  }
  
//테스트:
  //http://l
  //=> 리턴되는 콘텐트의 mime 타이보가 charset을 지정하고 싶다면
 //애노테이션의 produces 프로퍼티에 설정하라
  @GetMapping(value="h6", produces="text/html;charset=UTF-8")
  @ResponseBody 
  public HttpEntity<String> handler6(HttpServletResponse response) {
    //한글을 제데로 출력하고 싶으면
    //응답 헤더에 직접 Context-
    
    HttpHeaders headers = new HttpHeaders();
    headers.add("Content-Type", "text/html;charset=UTF-8");
    
    HttpEntity<String> entity = new HttpEntity(
        "<html><body><h1>abc가각간</h1></body></html>",
        headers);
    
  
       
    return entity;
   
    
  }
  
  //테스트:
    //http://l
    //=> 리턴되는 콘텐트의 mime 타이보가 charset을 지정하고 싶다면
   //애노테이션의 produces 프로퍼티에 설정하라
    @GetMapping(value="h7", produces="text/html;charset=UTF-8")
    @ResponseBody 
    public ResponseEntity<String> handler7(HttpServletResponse response) {
      //HttpEntity 대신에 ResponseEntity객체를 리턴할수 있다.
      //
      
      HttpHeaders headers = new HttpHeaders();
      headers.add("Content-Type", "text/html;charset=UTF-8");
      
      //이렇게 응답 헤더를 따로 설정하는 방법이 존재하는 이유는
      // 임의의 응답 헤더를 추가하는 경우가 있기 때문이다.
      //임의의 응답을 웹브라우저 쪽에 보낼수 있다.
      headers.add("BIT-OK", "ohara");
      
      ResponseEntity<String> entity = new ResponseEntity(
          "<html><body><h1>abc가각간</h1></body></html>",
          headers,
         HttpStatus.OK); //응답 헤더를 지정할수 있다.
               
      return entity;
     
      
    }
}





