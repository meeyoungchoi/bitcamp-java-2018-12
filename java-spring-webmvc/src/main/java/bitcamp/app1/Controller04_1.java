// 요청 핸들러의 파라미터 -프론트 컨트롤러로부터 받을 수있는 파라미터 값
...
public class Controller04_1 {
  ...
  //프론트 컨트롤러로부터 받고 싶은 값이 있다면
  //요청 핸들러를 정의할 때 받고 싶은 타입의 아규먼트를 선언하라!
  //그러면 프론트 컨트롤러가 메서드를 호출할 때 타입의 값을 넘겨줄 것이다.
  
  //ServletContext는 의존 객체로 주입 받아야 한다,
  //요청 핸들러에서 아규먼트로 받을 수 없다.
  @Autowired ServletContext sc;
  
  //요청 핸들러에서 받을 수 있는 타입의 아규먼트를 선턴해 보자!
  @PostMapping("h1") 
  @ResponseBody 
  public void handler1(
      ServletContext sc,//서블릿 컨텍스트는 파라미터로 받을 수 없다. 예외 발생
      ...
      Model model,//map과 같다.
      PrintWriter out   //클라잉언트에게 콘텐트를 보닐 때 사용할 출력 스트림
      ) {
    //ServletReqeust 와 ServletResponse 객체를 받을 수 있다.
