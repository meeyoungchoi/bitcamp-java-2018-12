...
//서블릿 클래스를 만든 후에 배치 파일(web.xml; DD파일)에 
//서블릿 정보를 등록해야만 실행될 수 있다.
//WEB-INF/web.xml
//
//서블릿 실행 방법
//=>http://서버주소:포트번호/웹애플리케이션이름/서블릿url
//예) http://localhost
//종료할때는 서버에서 stop을 누른다.
//
//서블릿 구동 과정
//1) 웹 브라우저가 서블릿 실행을 요청한다.
//2)서블릿 컨테이너는 해당 url에 서블릿 객체를 찾는다.
//3.1) 서블릿 객체가 없다면  
...
//  =>service()를 호출한다.
//
//만약 웹 애플리케이션이 종료되ㅣㄴ다면
//  =>생성된 모든 서블릿들의 destroy() 메서드를 호출한다.
//
//결론!
//=>특별한 옵션을 주지 않으면 클라이언트가 최조로 요청했을 때 서블릿 인스턴스를 생성하낟.
...
//=>클라이언트 마다 구분되어야 할 데이터는 서블릿 인스턴스 변수에 보관해서는 안된다.
//왜? 인스턴스는 모든 클라이언트가 공유하기 때문이다.(클라이언트가 인스턴스를 덮어쓸수 있기때문에)
public class Servlet01 implements Servlet {
 ...
 // 클라이언트가 이 서블릿의 실행을 요청할 때마다 호출된다.
    // 클라이언트가 요청한 작업을 수행한다.
    System.out.println("Servlet01.service(ServletRequest req, ServletResponse res)");
