...
public class Servlet01 extends HttpServlet {
  ...
  protected void service(
    ...
    /*
     * HTTP/1.1 200
Set-Cookie: JSESSIONID=A8EFB6E695543AAF9579DA60E4FF39BA; Path=/java-web; HttpOnly
Content-Type: text/plain;charset=UTF-8
Content-Length: 44
Date: Wed, 03 Apr 2019 02:38:43 GMT
     * 
     * 
     * 
     * */
    ...
    // -있으면, 그 세션을 리턴한다.
    // -있는데 세션의 유효기간이 지났다면,  새로 세션을 만들어 리턴한다.
    // - 없다면(즉 서버를 리스타트 해서 세션이 사라진 상태), 새로 세션을 만들어 리턴한다.
    //2) 클라잉너트가 세션 아이디를 보내지 않은 경우
    //-서버는 새 세션을 만들어 리턴한다.
    //항상 새 세션을 만들면, 응답할 때 새 세션의 아디디를 쿠키로 
    //서버가 찾아보니까 서버에 이미 세션이 있기때문에 새로 안만들고 쿠키를 보내지 않는다.
    ...
    out.println("세션을 생성 하였습니다. -/ex11/s1");  
  }
}
